...
import pandas as pd
cwd = os.getcwd()
path = glob.glob(cwd+"/"+"*_merge_sgRNA_nc_junction_summary.tsv")
...
for i in path:
    RAW_TSV = pd.read_csv(i,sep="\t",names = ["j5","j3","count","JS","deletion size",
        "sample","RPM","startpos"])
    TSV_summary = RAW_TSV.groupby('JS').agg({'count': 'sum','RPM':'sum','sample':'count',"j5":"first","j3":"first"}).sort_values(by='sample', ascending=False).reset_index()
    TSV_summary.columns = ['JS', 'number_of_reads', 'RPM','number_of_samples','j5','j3']
    ## Filter datasets 
