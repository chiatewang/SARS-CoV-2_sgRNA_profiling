...
Omicron_plot <- venn.diagram(list(B.1.1.529=B.1.1.529$JS,
                      ...
                      BA.5=BA.5$JS),
                      filename = NULL,
                      main = "Noncanonical sgRNAs from Different Omicron Lineages",
                     fill =c("navy","orange","yellow","coral","grey"))
grid.newpage()
grid.draw(Omicron_plot)
ggsave(file.path("./Omicron_venndiagram.pdf"), Omicron_plot, width = 20, height = 13, units = "cm")
...
Variant_plot <- venn.diagram(list(Omicron=Omicron$JS,
                             ...
                             filename = NULL,
                             main.cex = 1,
                             main = "Noncanonical sgRNAs from Different Variants",fill=c("navy","orange","yellow","coral","grey"))
grid.newpage()
grid.draw(Variant_plot)
ggsave(file.path("./Five_Variant_venndiagram.pdf"), Variant_plot, width = 20, height = 13, units = "cm")
```
...
## generate venn diagram (Omicron)
Omicron_plot <- venn.diagram(list(B.1.1.529=B.1.1.529_TOP50$JS,
                      BA.1=BA.1_TOP50$JS,
                      BA.2=BA.2_TOP50$JS,
                      BA.4=BA.4_TOP50$JS,
                      BA.5=BA.5_TOP50$JS),
                     filename = NULL,
                     main = "Top 50 ncsgRNAs from Different Omicron Lineages",
                     main.cex = 1,
                     fill =c("navy","orange","yellow","coral","grey"))
grid.newpage()
grid.draw(Omicron_plot)
ggsave(file.path("./Top50_Omicron_venndiagram.pdf"), Omicron_plot, width = 20, height = 13, units = "cm")
...
## generate venn diagram (five variants)
Variant_plot <- venn.diagram(list(Omicron=Omicron_TOP50$JS,
                                  Beta=Beta_TOP50$JS,
                                  Alpha=Alpha_TOP50$JS,
                                  Gamma=Gamma_TOP50$JS,
                                  Delta=Delta_TOP50$JS),
                             filename = NULL,
                             main = "Top 50 ncsgRNAs from Different Variants",main.cex = 1,fill=c("navy","orange","yellow","coral","grey"))
grid.newpage()
grid.draw(Variant_plot)
ggsave(file.path("./Top50_Five_Variant_venndiagram.pdf"), Variant_plot, width = 20, height = 13, units = "cm")
```
...
ggsave(file.path("./BA.4_BA.5.pdf"), full, width = 20, height = 13, units = "cm")
```
...
Variant=rbind(Alpha,Omicron)

summary = Variant%>%mutate(total=(as.numeric(as.character(count))))%>%group_by(j5,y,location,j3)%>%summarise(n=n(),m=sum(total))

full=ggplot(mapping=aes(x=j5,y=y,xend=j3,yend=y))+
  geom_curve(data=summary%>%filter(location=="Alpha")%>%filter(n>20),lineend = "round",curvature=0.3,color=grey,size=0.1)+
  geom_curve(data=summary%>%filter(location=="Omicron")%>%filter(n>20),lineend = "round",curvature=-0.3,color=grey,size=0.1)+
  scale_alpha_continuous("Number of Samples")+
  ft_theme()+
  scale_x_continuous("Position")+
  annotate(geom="rect",xmin=0,xmax=30000,ymin=-0.05,ymax=0.05,color="grey80",fill="grey80")+
  ...
  annotate(geom="rect",xmin=29510,xmax=30000,ymin=-0.05,ymax=0.05,color="grey80",fill="grey80")+
  annotate(geom="rect",xmin=0,xmax=30000,ymin=-0.05,ymax=0.05,fill="white",alpha=0.3)+
  geom_point(alpha=0.8,data=summary%>%filter(location=="Alpha")%>%filter(n>20)%>%arrange(n),aes(x=j5,y=-0.05,size=m,color=n))+
  geom_point(alpha=0.8,data=summary%>%filter(location=="Omicron")%>%filter(n>20),aes(x=j5,y=0.05,size=m,color=n))+
  geom_point(alpha=0.8,data=summary%>%filter(location=="Alpha")%>%filter(n>20)%>%arrange(n),aes(x=j3,y=-0.05,size=m,color=n))+
  geom_point(alpha=0.8,data=summary%>%filter(location=="Omicron")%>%filter(n>20),aes(x=j3,y=0.05,size=m,color=n))+
  theme(
    axis.text.y=element_blank(),
    axis.title.y=element_blank(),
    axis.line.y = element_blank(),
...
ggsave(file.path("./Alpha_Omicron.pdf"), full, width = 20, height = 13, units = "cm")
```
...
Variant=rbind(Beta,Omicron)

summary = Variant%>%mutate(total=(as.numeric(as.character(count))))%>%group_by(j5,y,location,j3)%>%summarise(n=n(),m=sum(total))

full=ggplot(mapping=aes(x=j5,y=y,xend=j3,yend=y))+
  geom_curve(data=summary%>%filter(location=="Beta")%>%filter(n>20),lineend = "round",curvature=0.3,color=grey,size=0.1)+
  geom_curve(data=summary%>%filter(location=="Omicron")%>%filter(n>20),lineend = "round",curvature=-0.3,color=grey,size=0.1)+
  scale_alpha_continuous("Number of Samples")+
  ft_theme()+
  scale_x_continuous("Position")+
  ...
  annotate(geom="rect",xmin=29510,xmax=30000,ymin=-0.05,ymax=0.05,color="grey80",fill="grey80")+
  annotate(geom="rect",xmin=0,xmax=30000,ymin=-0.05,ymax=0.05,fill="white",alpha=0.3)+
  geom_point(alpha=0.8,data=summary%>%filter(location=="Beta")%>%filter(n>20)%>%arrange(n),aes(x=j5,y=-0.05,size=m,color=n))+
  geom_point(alpha=0.8,data=summary%>%filter(location=="Omicron")%>%filter(n>20),aes(x=j5,y=0.05,size=m,color=n))+
  geom_point(alpha=0.8,data=summary%>%filter(location=="Beta")%>%filter(n>20)%>%arrange(n),aes(x=j3,y=-0.05,size=m,color=n))+
  geom_point(alpha=0.8,data=summary%>%filter(location=="Omicron")%>%filter(n>20),aes(x=j3,y=0.05,size=m,color=n))+
  theme(
    axis.text.y=element_blank(),
    axis.title.y=element_blank(),
...
Variant=rbind(Gamma,Omicron)

summary = Variant%>%mutate(total=(as.numeric(as.character(count))))%>%group_by(j5,y,location,j3)%>%summarise(n=n(),m=sum(total))

full=ggplot(mapping=aes(x=j5,y=y,xend=j3,yend=y))+
  geom_curve(data=summary%>%filter(location=="Gamma")%>%filter(n>20),lineend = "round",curvature=0.3,color=grey,size=0.1)+
  geom_curve(data=summary%>%filter(location=="Omicron")%>%filter(n>20),lineend = "round",curvature=-0.3,color=grey,size=0.1)+
  scale_alpha_continuous("Number of Samples")+
  ft_theme()+
  scale_x_continuous("Position")+
  ...
  annotate(geom="rect",xmin=29510,xmax=30000,ymin=-0.05,ymax=0.05,color="grey80",fill="grey80")+
  annotate(geom="rect",xmin=0,xmax=30000,ymin=-0.05,ymax=0.05,fill="white",alpha=0.3)+
  geom_point(alpha=0.8,data=summary%>%filter(location=="Gamma")%>%filter(n>20)%>%arrange(n),aes(x=j5,y=-0.05,size=m,color=n))+
  geom_point(alpha=0.8,data=summary%>%filter(location=="Omicron")%>%filter(n>20),aes(x=j5,y=0.05,size=m,color=n))+
  geom_point(alpha=0.8,data=summary%>%filter(location=="Gamma")%>%filter(n>20)%>%arrange(n),aes(x=j3,y=-0.05,size=m,color=n))+
  geom_point(alpha=0.8,data=summary%>%filter(location=="Omicron")%>%filter(n>20),aes(x=j3,y=0.05,size=m,color=n))+
  theme(
    axis.text.y=element_blank(),
    axis.title.y=element_blank(),
...
ggsave(file.path("./Gamma_Omicron.pdf"), full, width = 20, height = 13, units = "cm")
```
...
Variant=rbind(Delta,Omicron)

summary = Variant%>%mutate(total=(as.numeric(as.character(count))))%>%group_by(j5,y,location,j3)%>%summarise(n=n(),m=sum(total))

full=ggplot(mapping=aes(x=j5,y=y,xend=j3,yend=y))+
  geom_curve(data=summary%>%filter(location=="Delta")%>%filter(n>20),lineend = "round",curvature=0.3,color=grey,size=0.1)+
  geom_curve(data=summary%>%filter(location=="Omicron")%>%filter(n>20),lineend = "round",curvature=-0.3,color=grey,size=0.1)+
  scale_alpha_continuous("Number of Samples")+
  ft_theme()+
  scale_x_continuous("Position")+
  ...
  annotate(geom="rect",xmin=29510,xmax=30000,ymin=-0.05,ymax=0.05,color="grey80",fill="grey80")+
  annotate(geom="rect",xmin=0,xmax=30000,ymin=-0.05,ymax=0.05,fill="white",alpha=0.3)+
  geom_point(alpha=0.8,data=summary%>%filter(location=="Delta")%>%filter(n>20)%>%arrange(n),aes(x=j5,y=-0.05,size=m,color=n))+
  geom_point(alpha=0.8,data=summary%>%filter(location=="Omicron")%>%filter(n>20),aes(x=j5,y=0.05,size=m,color=n))+
  geom_point(alpha=0.8,data=summary%>%filter(location=="Delta")%>%filter(n>20)%>%arrange(n),aes(x=j3,y=-0.05,size=m,color=n))+
  geom_point(alpha=0.8,data=summary%>%filter(location=="Omicron")%>%filter(n>20),aes(x=j3,y=0.05,size=m,color=n))+
  theme(
    axis.text.y=element_blank(),
    axis.title.y=element_blank(),
...
ggsave(file.path("./Delta_Omicron.pdf"), full, width = 20, height = 13, units = "cm")
```
...
```{r}
BA.4 = read.delim("/Users/justin/Desktop/SPAN/tsv/BA.4_sgRNA_nc_junction_summary.tsv", header= TRUE, sep = "\t")
...
BA.5 = read.delim("/Users/justin/Desktop/SPAN/tsv/BA.5_sgRNA_nc_junction_summary.tsv", header= TRUE, sep = "\t")
