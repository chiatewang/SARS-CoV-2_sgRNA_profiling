...
# 2. Get annotation file (這邊可以修改新的annotation file)
import pandas as pd
annotations = pd.read_csv('/home/M1013001/ref/SARS-CoV-2-annotations.gff', sep='\t',
                         names=['chrom', 'type', 'name', 'start', 'end', '_x', '_y', '_z'])
annotations['start'] -= 1

# cds without Leader sequence
...
covseq = str(next(SeqIO.parse('/home/M1013001/ref/sars_cov2_NC_045512.2_genome.fasta','fasta')).seq)
...
## 6. Echo output data
junction_anno = pd.DataFrame(junction_anno, columns=[
    'j5', 'j3', 'count', 'startpos', 'productsize'])
junction_anno = junction_anno.sort_values(by='count', ascending=False).reset_index(drop=True)
junction_mg = junction_anno.groupby('startpos').agg({'count': 'sum', 'j5': 'first', 'j3': 'first',
                                      'productsize': 'first'}).sort_values(by='count',
                                                                          ascending=False)

#-------------------------------------------------------------------------------------------------#

## 7. Classify canonical sgRNA
...
## 8. Classify TRS-L noncanonical sgRNA
...
## 8-1. Test if startcodon is generated by the fusion
...
if can_nonprod['count'].sum() > 0 :
    if fusestarted['count'].sum() == 0 :
        ...
        fusestarted = fusestarted.reindex(columns=['j5','j3','count','startpos','productsize','chrom','type','name','start','end','evtype','evrelorf','evsize','startpos_byfusion']).head()
...
# 8-2. Classify TRS-L dependent noncanonical sgRNA (inframe or outframe)
if can_nonprod['count'].sum() > 0 :
    if remaining['count'].sum() == 0 :
...
## 9. Classify TRS-L independent Non-canonical sgRNA
...
## 想改成inter or intra-gene的模式 取代distal or proximal
...
# 9-1. Classify TRS-L independent noncanonical sgRNA (inframe or outframe)
...
# 10. Noncanonical sgRNA count summary
...
ncsg_count.to_csv(FileNamePrefix + "nc_sgRNA_count.tsv", sep="\t", mode='a+',index=False)

#-------------------------------------------------------------------------------------------------#

## 11. Generate sgRNA junction summary file
#sgRNA_summary = []
#sgRNA_summary = pd.DataFrame(sgRNA_summary, columns= ['j5', 'j3', 'count','startpos', 'productsize', 'chorm' ,'type' ,'name' ,'start' ,'end','evtype', 'evrelorf', 'evsize','startpos_byfusion'])
...
can_prod.to_csv(FileNamePrefix + "sgRNA_junction_summary.tsv", sep="\t",mode='a+',header=0,index=False)
if can_nonprod['count'].sum() > 0 :
    if fusestarted['count'].sum() == 0 :
        print("no fusestarted")
        remaining.to_csv(FileNamePrefix + "sgRNA_junction_summary.tsv", sep="\t", mode='a+',header=0,index=False)
    else : 
        fusestarted.to_csv(FileNamePrefix + "sgRNA_junction_summary.tsv", sep="\t", mode='a+',header=0,index=False)
else :
    print("no can_nonprod")
nc_distal.to_csv(FileNamePrefix + "sgRNA_junction_summary.tsv", sep="\t",mode='a+',header=0,index=False)
nc_proximal.to_csv(FileNamePrefix + "sgRNA_junction_summary.tsv", sep="\t",mode='a+',header=0,index=False)
