...
usage() {
        ...
        3. sgRNA Percentage Plot
         Generating boxplot to visualize each types of sgRNAs by sgRNA_percentage.py adapted from Kim et al.
...
# Capture Acc number and set a variable
    ...
    do
        ...
        ILLUMINACLIP:adapters/TruSeq3-SE:2:30:10 \
        LEADING:3 \
        TRAILING:3 \
        SLIDINGWINDOW:4:15 \
        MINLEN:36
...
python ./SPAN/script/python/sgRNA_classification.py \
-i `echo ./"$VARIANTS"_SJ.out.tab` \
-n "$VARIANTS"_

echo "4. sgRNA Percentage Plot"

python ./SPAN/script/python/sgRNA_percentage.py \
